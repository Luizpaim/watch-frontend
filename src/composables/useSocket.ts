...
export const useSocket = () => {
        ...
        (newToken) => {
            ...
            if (newToken) {
                ...
                socket.on("disconnect", () => {
                    console.warn("Websocket desconnected")
                })
            }
        },
        { immediate: true }
    ...
    const on = (event: string, callback: (...args: Record<string, any>[]) => void) => {
        currentSocket.value?.on(event, callback)
    }

    const off = (event: string, callback: (...args: Record<string, any>[]) => void) => {
